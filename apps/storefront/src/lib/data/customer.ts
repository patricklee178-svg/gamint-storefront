...
export async function uploadAvatar(
  ...
  if (file.size > 3 * 1024 * 1024) {
    return { success: false, error: "حجم فایل باید کمتر از ۳ مگابایت باشد." }
  }
  ...
  try {
    res = await fetch(`${MEDUSA_BACKEND_URL}/store/customers/me/avatar`, {
      method: "POST",
      headers: {
        ...authHeaders,
  ...
  if (!res.ok) {
    ...
    return { success: false, error: body?.message || "خطا در آپلود تصویر." }
  }
...
export async function removeAvatar(): Promise<{ success: boolean; error?: string }> {
  ...
  const res = await fetch(`${MEDUSA_BACKEND_URL}/store/customers/me/avatar`, {
    method: "DELETE",
    headers: {
      ...authHeaders,
      "x-publishable-api-key": process.env.NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY || "",
